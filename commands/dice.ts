    ...
    execute() {
        ...
        if (user.balance >= bet) {
            ...
            if (roll === 6) {
                user.balance += bet * 5;
                user.xp += 100;
                this.interaction.followUp({ embeds: [this.generateEmbed(bet, true, user.balance, roll)], empherial: false })
            }
            else {
                user.balance -= bet;
                user.xp += 10;
                this.interaction.followUp({ embeds: [this.generateEmbed(bet, false, user.balance, roll)], empherial: false })
            }
